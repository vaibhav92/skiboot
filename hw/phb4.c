...
static void disable_capi_mode(struct phb4 *p)
{
	...
	xscom_write(p->chip_id, capp->capp_xscom_offset +
		    CAPP_ERR_STATUS_CTRL, PPC_BIT(3) | PPC_BIT(4));
	...
	xscom_write(p->chip_id, capp->capp_xscom_offset +
		    APC_FSM_READ_MASK, 0ull);
	xscom_write(p->chip_id, capp->capp_xscom_offset +
		    XPT_FSM_RMM, 0ull);

	/* Disable snoop */
	xscom_write(p->chip_id, capp->capp_xscom_offset + SNOOP_CAPI_CONFIG, 0);

	/* Clear flush SUE state map register */
	xscom_write(p->chip_id, capp->capp_xscom_offset +
		    FLUSH_SUE_STATE_MAP, 0);

	/* Disable epoch timer */
	xscom_write(p->chip_id, capp->capp_xscom_offset +
		    EPOCH_RECOVERY_TIMERS_CTRL, 0);

	/* CAPP Transport Control Register */
	xscom_write(p->chip_id, capp->capp_xscom_offset +
		    TRANSPORT_CONTROL, PPC_BIT(15));
	...
	xscom_write(p->chip_id, capp->capp_xscom_offset + CAPP_ERR_RPT_CLR, 0);
	xscom_write(p->chip_id, capp->capp_xscom_offset + CAPP_FIR, 0);
	xscom_write(p->chip_id, capp->capp_xscom_offset + CAPP_FIR_ACTION0, 0);
	xscom_write(p->chip_id, capp->capp_xscom_offset + CAPP_FIR_ACTION1, 0);
	xscom_write(p->chip_id, capp->capp_xscom_offset + CAPP_FIR_MASK, 0);
				  ...
				  struct capp_info *info)
{
	...
	if (phb->capp == NULL || phb->capp->phb != phb)
		return OPAL_PARAMETER;
				  ...
				  uint64_t pe_number)
{
	...
	case OPAL_PHB_CAPI_MODE_SNOOP_ON:
		...
		ret = phb->capp ? OPAL_SUCCESS : OPAL_UNSUPPORTED;
		break;

	case OPAL_PHB_CAPI_MODE_SNOOP_OFF:
		ret = phb->capp ? OPAL_UNSUPPORTED : OPAL_SUCCESS;
		break;
